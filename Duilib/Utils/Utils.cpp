...
CDuiPoint::CDuiPoint(LPCTSTR pstrValue) {
    ...
    LPTSTR pstr = NULL;
    x = y = _tcstol(pstrValue, &pstr, 10);
    ASSERT(pstr);
    y = _tcstol(pstr + 1, &pstr, 10);
    ASSERT(pstr);
}
...
CDuiSize::CDuiSize(LPCTSTR pstrValue) {
    ...
    LPTSTR pstr = NULL;
    cx = cy = _tcstol(pstrValue, &pstr, 10);
    ASSERT(pstr);
...
CDuiRect::CDuiRect(LPCTSTR pstrValue) {
    ...
    LPTSTR pstr = NULL;
    left = top = right = bottom = _tcstol(pstrValue, &pstr, 10);
    ASSERT(pstr);
    top = bottom = _tcstol(pstr + 1, &pstr, 10);
    ASSERT(pstr);
    right = _tcstol(pstr + 1, &pstr, 10);
    ASSERT(pstr);
    bottom = _tcstol(pstr + 1, &pstr, 10);
    ASSERT(pstr);
}
