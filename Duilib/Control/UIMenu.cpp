...
LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CLOSE:
        if (m_pOwner != NULL) {
            ...
            m_pOwner->SetPos(m_pOwner->GetPos(), true);
            m_pOwner->SetFocus();
        }
        break;
    case WM_RBUTTONDOWN:
...
void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem) {
    if (m_bShowExplandIcon) {
        LPCTSTR strIcon = m_pManager->GetDefaultAttributeList(_T("ExplandIcon"));
        if (!(m_expandIcon.sDrawString == strIcon && m_expandIcon.pImageInfo != NULL) ) {
            m_expandIcon.Clear();
            m_expandIcon.sDrawString = strIcon;
        }
        DrawImage(hDC, m_expandIcon);
    }
}
        ...
        if (IsEnabled()) {
            ...
            if (hasSubMenu) {
                ...
                if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL) {
                    ...
                    PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)pMenuCmd, (LPARAM)this);
                }
                ContextMenuParam param;
                param.hWnd = m_pManager->GetPaintWindow();
                param.wParam = 1;
...
void CMenuElementUI::CreateMenuWnd() {
    ...
    m_pWindow->Init(static_cast<CMenuElementUI*>(this), _T(""), CDuiPoint(), NULL, NULL, eMenuAlignment_Left | eMenuAlignment_Top);
}
...
void CMenuElementUI::SetIcon(LPCTSTR strIcon) {
    if (m_icon.sDrawString == strIcon && m_icon.pImageInfo != NULL) return;
    m_icon.Clear();
    m_icon.sDrawString = strIcon;
    Invalidate();
}
