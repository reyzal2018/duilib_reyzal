...
namespace DuiLib {
CProgressUI::CProgressUI() : m_bHorizontal(true), m_bStretchForeImage(true), m_nMin(0), m_nMax(100), m_nValue(0), m_sForeImage(_T("")) {
...
void CProgressUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
    ...
    else if (_tcscmp(pstrName, _T("stretchforeimage")) == 0) SetStretchForeImage(_tcsicmp(pstrValue, _T("true")) == 0 ? true : false);
...
void CProgressUI::PaintStatusImage(HDC hDC) {
    ...
    if (IsStretchForeImage()) {
        m_diFore.rcDestOffset = rc;
        m_diFore.sDrawString = m_sForeImage;
    } else {
        //只能是bkimage=xx模式
        CDuiString strNewDrawString;
        strNewDrawString.SmallFormat(_T("file='%s' source='%d,%d,%d,%d' dest='%d,%d,%d,%d'"), (LPCTSTR)m_sForeImage, rc.left, rc.top, rc.right, rc.bottom, rc.left, rc.top, rc.right, rc.bottom);
        m_diFore.sDrawString = strNewDrawString;
    }
