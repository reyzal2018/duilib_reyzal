        ...
        while( *pstrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") ) {
                    ...
                    drawInfo.rcDestOffset.left = _tcstol(sValue.GetData(), &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcDestOffset.top = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcDestOffset.right = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcDestOffset.bottom = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                } else if( sItem == _T("source") ) {
                    drawInfo.rcBmpPart.left = _tcstol(sValue.GetData(), &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcBmpPart.top = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcBmpPart.right = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcBmpPart.bottom = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                } else if( sItem == _T("corner") || sItem == _T("scale9")) {
                    drawInfo.rcScale9.left = _tcstol(sValue.GetData(), &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcScale9.top = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcScale9.right = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
                    drawInfo.rcScale9.bottom = _tcstol(pstr + 1, &pstr, 10);
                    ASSERT(pstr);
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, int iDefaultFont, UINT uStyle) {
    ...
    CDuiArray<LPVOID>	aFontArray(10);
    CDuiArray<DWORD>	aColorArray(10);
    CDuiArray<int>		aPIndentArray(10);
    CDuiArray<UINT>		aVAlignArray(10);
    ...
    CDuiArray<LPVOID>	aLineFontArray;
    CDuiArray<DWORD>	aLineColorArray;
    CDuiArray<int>		aLinePIndentArray;
    CDuiArray<UINT>		aLineVAlignArray;
